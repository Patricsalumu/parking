...
<form method="POST" action="{{ route('comptes.update', $compte) }}">
  @csrf @method('PUT')
  <div class="mb-3"><label>Numéro</label><input name="numero" value="{{ old('numero', $compte->numero) }}" class="form-control"></div>
  <div class="mb-3"><label>Intitulé</label><input name="nom" value="{{ old('nom', $compte->nom) }}" class="form-control"></div>
  <div class="mb-3"><label>Classe</label>
    <select name="classe_id" class="form-control">
      @foreach($classes as $cl)
        <option value="{{ $cl->id }}" {{ $compte->classe_id == $cl->id ? 'selected' : '' }}>{{ $cl->numero ?? $cl->id }} - {{ $cl->nom }}</option>
      @endforeach
    </select>
  </div>
