...
<form method="POST" action="{{ route('entrees.store') }}">
  ...
  <div class="mb-3">
    <label>Marque</label>
    <input name="marque" id="marque" value="{{ old('marque') }}" class="form-control" placeholder="Vehicle make/model">
  </div>

  <div class="mb-3">
    <label>Pays <span class="text-danger">*</span></label>
    <input name="pays" id="pays" value="{{ old('pays') }}" class="form-control" placeholder="Country" required>
  </div>
  ...
  <div class="mb-3"><label>QR Code (optional)</label><input name="qr_code" class="form-control" value="{{ old('qr_code') }}"></div>
